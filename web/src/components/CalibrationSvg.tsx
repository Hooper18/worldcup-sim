...
export default function CalibrationSvg({ points }: Props) {
  ...
  return (
    <svg viewBox={`0 0 ${S} ${S}`} className="w-full max-w-[260px]" role="img">
      {/* 对角线（完美校准） */}
      <line x1={x(0)} y1={y(0)} x2={x(1)} y2={y(1)} stroke="#E7E4DE" strokeWidth={1} strokeDasharray="3 3" />
      {/* 轴 */}
      <line x1={pad} y1={S - pad} x2={S - pad} y2={S - pad} stroke="#E7E4DE" />
      <line x1={pad} y1={pad} x2={pad} y2={S - pad} stroke="#E7E4DE" />
      <text x={S / 2} y={S - 6} textAnchor="middle" fontSize={10} fill="#9C9A96">预测概率</text>
      <text x={10} y={S / 2} textAnchor="middle" fontSize={10} fill="#9C9A96" transform={`rotate(-90 10 ${S / 2})`}>
        实际频率
      </text>
      {/* 点（大小 ~ 样本数） */}
      {points.map((p, i) => (
        <circle key={i} cx={x(p[0])} cy={y(p[1])} r={3 + 4 * (p[2] / maxN)} fill="#2F6B4F" fillOpacity={0.55} />
      ))}
    </svg>
  )
}
